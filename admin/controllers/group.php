 ...
 * @since       1.6
 */
class HecMailingControllerGroup extends JControllerForm
{
	/**
	 * @var	    string  The prefix to use with controller messages.
	 * @since   1.6
	 */
	protected $text_prefix = 'COM_HECMAILING_GROUP';
	...
	protected function _save($task)
	{
		...
		$input = $app->input;
		$data=$_POST;
		$data['import_file'] = $_FILES['import_file'];
		if ($model->save($data))
		{
			$msg = JText::_("COM_HECMAILING_GROUPE_SAVED");
			if ($task=="apply")
			{
			...
			{
				$link=  JRoute::_('index.php?option=com_hecmailing&view=group&layout=edit&id=-'.$data['id'],FALSE);
				$app->redirect ($link, $msg);
		...
		{
			$msg = JText::sprintf("COM_HECMAILING_GROUPE_NOTSAVED", $model->error);
	...
	public function delete()
	{
		...
		if ($model->delete($cid))
		{
			$msg = JText::_("COM_HECMAILING_GROUP_DELETED");
			$link=  JRoute::_('index.php?option=com_hecmailing&view=groups',FALSE);
			$app->redirect ($link, $msg);
		}
		else
		{
			$msg = JText::sprintf("COM_HECMAILING_GROUP_NOTDELETED", $model->error);
			$app->enqueueMessage($msg,'error');
